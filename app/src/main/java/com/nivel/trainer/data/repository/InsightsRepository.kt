...
/**
 * Результат действия ревью карточки (A2, #96): одобрить/отклонить/редактировать.
...
interface InsightsRepository {
    ...
    /** Одобрить draft-карточку (A2). */
    suspend fun approveCard(cardId: String): CardActionResult

    /** Отклонить draft-карточку (A2). */
    suspend fun rejectCard(cardId: String): CardActionResult

    /** Отредактировать карточку (title/body/tag/side). Валидация — на сервере (A2). */
