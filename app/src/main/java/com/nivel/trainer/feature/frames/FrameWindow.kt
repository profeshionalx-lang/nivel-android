...
object FrameWindow {
    ...
    /**
     * Шаг линейного разбора окна (#117) — кэш для скраба слайдера и источник плёнки.
     * 100мс — минимальный шаг, при котором момент удара (сотые доли секунды) ещё
     * различим; секундная сетка (которую владелец тоже рассматривал) его теряет.
     */
    const val SCAN_STEP_MS = 100L

    /** Порог подвыборки плёнки из плотного кэша скана — не каждый кадр, а раз в ~250мс. */
