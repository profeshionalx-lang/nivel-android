...
/**
 * Ненавязчивый баннер «работаем оффлайн» поверх кэшированных данных (G3, #71). Показывается,
 * когда обновление с сервера сорвалось (сеть, 401/500/иное), но кэш есть — данные на экране
 * устарели, но валидны. Кнопка «Обновить» повторяет запрос. Mobile-first: тач-зона ≥48dp.
 * Текст зависит от причины: [isNetworkError] → «нет сети», иначе — нейтральное «не удалось».
 */
@Composable
fun OfflineBanner(
    onRetry: () -> Unit,
